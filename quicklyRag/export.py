...
def vectorize_file(
    ...
    try:
        ...
        # 1. 加载文档
        logger.info(f"文件 {path_obj} 正在加载文档...")
        document = load_document(file_path=path_obj)
        logger.success(f"文件 {path_obj} 加载成功, 文档类型: {type(document)}, 文档数量: {len(document) if hasattr(document, '__len__') else 'N/A'}")
        
        # 2. 拆分文档
        logger.info(f"文件 {path_obj} 正在拆分...  ")
        documents = spliter_file(document, rag_config=rag_config)
        logger.success(f"文件 {path_obj} 拆分成功, 拆分段数: {len(documents)}")
        
        # 3. 向量化文档
        logger.info(f"文件 {path_obj} 正在进行文档向量化...")
        embeds = embed_document(documents, embedding_type)
        logger.success(f"文件 {path_obj} 向量化成功, 向量数量: {len(embeds)}, 向量维度: {len(embeds[0]) if embeds and len(embeds) > 0 else 'N/A'}")
        
        # 4. 存储向量
        logger.info(f"文件 {path_obj} 正在存储向量到数据库...")
        store_vector_by_documents(documents, vectorstore_type)
        logger.success(f"文件 {path_obj} 文档存储成功，共存储 {len(documents)} 个文档片段")
